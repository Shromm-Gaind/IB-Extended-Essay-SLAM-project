...
for i in range(len(t_lidar) - 1):
    # print("{} \n".format(i))
    # prediction
    # As we have a lot of data from sensor we can set our prediction at previos value
    # in this case our Q matrix will have physical meaning of maximum posibility to change yaw during one time step
    x = matrix([[yaw[i]]]) + matrix([[(wYaw_gyro[i] + wYaw_gyro[i + 1]) * 0.5 * (t_gyro[i + 1] - t_gyro[i])]])
    P = P_bel + Q
